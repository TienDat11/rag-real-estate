...
from ...application.services.sql_leg import SqlLegResult, build_dsn, with_rls_identity
...
async def run_nl2sql(query: str, as_of: date | None) -> SqlLegResult:
    ...
    model = _get_model_for_role("nl2sql")
    raw = await _get_llm_fn().complete(_build_messages(query, as_of), model=model, timeout=LLM_CALL_TIMEOUT_S)
    sql = extract_sql(raw)
    if not sql:
