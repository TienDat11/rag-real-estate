...
SSE event order: sources -> facts -> token -> done (error before done on failure).
"""
...
async def _sse_stream(pipe, req: QueryRequest, as_of_date) -> AsyncIterator[str]:
    ...
    async def run_pipe() -> None:
        try:
            payload = await pipe.run(req.query, req.session_id, as_of_date, history, on_event=on_event)
...
def create_app() -> FastAPI:
    ...
    async def query(req: QueryRequest, request: Request) -> "StreamingResponse | dict":
        from api.workflow import QueryRejected, RagQueryPipeline, parse_as_of  # noqa: PLC0415
        ...
        if "text/event-stream" in accept:
            ...
            return StreamingResponse(
                _sse_stream(pipe, req, as_of_date),
        ...
        try:
            payload = await pipe.run(
                req.query, req.session_id, as_of_date,
